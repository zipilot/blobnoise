...
assert.equal(manifest.name, "@zipilot/blobnoise");
const consumer = await mkdtemp(join(tmpdir(), "blobnoise-registry-"));

try {
  await writeFile(join(consumer, "package.json"), JSON.stringify({ private: true, type: "module" }));
  await writeFile(join(consumer, ".npmrc"), "@zipilot:registry=https://npm.pkg.github.com\n");
  execFileSync("npm", [
    "install", "--ignore-scripts", "--no-audit", "--no-fund",
    `${manifest.name}@${manifest.version}`, "react@19.3.0", "@types/react@19.3.0",
  ], { cwd: consumer, stdio: "inherit" });
  const lock = JSON.parse(await readFile(join(consumer, "package-lock.json"), "utf8"));
  const installed = lock.packages["node_modules/@zipilot/blobnoise"];
  assert.equal(installed.version, manifest.version);
  assert.equal(new URL(installed.resolved).hostname, "npm.pkg.github.com");

  execFileSync(process.execPath, ["--input-type=module", "-e", `
    import assert from "node:assert/strict";
    import { createConfig, createSnippet, parseConfig, serializeConfig } from "@zipilot/blobnoise";
    ...
    assert.ok(createSnippet(config).includes('from "@zipilot/blobnoise/browser"'));
  ...
  await writeFile(join(consumer, "consumer.ts"), `
    import { createConfig, type BlobConfig } from "@zipilot/blobnoise";
    import { createRenderer, type BlobRenderer } from "@zipilot/blobnoise/browser";
    import { exportWebP, exportWebM } from "@zipilot/blobnoise/export";
    import { BlobNoise, type BlobNoiseProps } from "@zipilot/blobnoise/react";

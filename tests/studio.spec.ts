...
test("decorative copy is absent and dialogs use functional labels", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Package setup" })).toHaveAttribute("href", "https://github.com/alejo-valencia/blobnoise#install-the-package");
...
test("full code/config export and unavailable clipboard report honestly", async ({ page }) => {
  ...
  await expect(snippet).toContainText('import { createRenderer } from "@alejo-valencia/blobnoise/browser"');
